...
class SteemConnectStrategy {
  constructor(options, verify) {
    ...
    if (!verify) {
      throw new TypeError("SteemConnectStrategy requires a verify callback");
    }
    if (!options.authorizationURL) {
      throw new TypeError(
        "SteemConnectStrategy requires a authorizationURL option"
      );
    }
    if (!options.tokenURL) {
      throw new TypeError("SteemConnectStrategy requires a tokenURL option");
    }
    if (!options.clientID) {
      throw new TypeError("SteemConnectStrategy requires a clientID option");
    }
    ...
    this._key =
      options.sessionKey ||
      `steemconnect:${url.parse(options.authorizationURL).hostname}`;
    if (options.store) {
      ...
      function loaded(err, ok, state) {
        ...
        self.getOAuthAccessToken(
          ...
          (err, accessToken, refreshToken, params) => {
            if (err !== null) {
              return self.error(
                self._createOAuthError(err.error_description, err)
              );
            }
  ...
  getOAuthAccessToken(code, params, callback) {
    const scope = this._scope ? this._scope.join(",") : "offline,vote";
    const url = scope.includes("offline")
      ? `${this._steemconnect
          ._accessTokenUrl}?refresh_token=${code}&scope=${scope}`
      : `${this._steemconnect
          ._accessTokenUrl}?access_token=${code}&scope=${scope}`;
    const getOAuth = new Promise((resolve, reject) => {
      fetch(url, {
        credentials: "include",
        headers: {
        ...
        method: "POST",
        body: JSON.stringify({
          client_secret: this._steemconnect._clientSecret
        })
      })
        .then(r => r.json())
        .then(steemConnectObj => {
    ...
    getOAuth
      .then(res => {
        if(res.status) {
          let err = {
            message: res.status,
            error_description: 'Enter A Valid Token URL',
          }
          ...
          callback(res, null, null, null, params);
        }
      })
      .catch(err => {
        callback(err, null, null, null, params);
